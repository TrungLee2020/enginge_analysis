...
```
- **Cấu trúc mediation (xem `docs/06`, `docs/07` §3–5):** phương trình này là **tầng 3** của cascade — `θ·(macro)` là phần INDIRECT (shock đã truyền qua tầng 2 global macro), `λ·VN-GPR⊥` là phần DIRECT. Tầng 2 (`γ_{M,j}`: GPR→Oil/DXY/VIX/US10Y) ước lượng riêng, generic. Tổng tác động phân rã `Total = Direct + Σ γ_{M,j}·θ_M` (KĐ12).
...
- **Split:** In-sample 2015–2022 (ước lượng), Out-of-sample 2023–2026 (kiểm định, KHÔNG chạm vào khi tune). Cửa sổ OOS chứa giai đoạn thuế quan 2025–26 — stress test tự nhiên tốt nhất.
...
- **Kiểm định 12 (mediation — direct vs indirect, xem `docs/06`):** phân rã tổng tác động GPR→VN-Index thành Direct (λ·VN-GPR⊥) và Indirect (Σ γ_{M,j}·θ_M, qua tầng 2 global macro); test ý nghĩa bằng bootstrap SE. Kỳ vọng Indirect ≫ Direct (nền KT nhỏ mở chịu shock chủ yếu qua kênh vĩ mô toàn cầu). Nếu Direct đáng kể → có rủi ro riêng VN không qua global, biện minh xây corpus tiếng Việt (V-phase). Gắn với cổng G2b.
