...
được kết quả đã publish thì lưới vô nghĩa. Đây là test loader + LP/HAC theo timing
headline của paper; đường M+1 có diagnostic riêng.
...
Chạy:  python scripts/run_e0_replication.py
"""
...
def build_ci_panel(aligned: bool = False) -> pd.DataFrame:
    """Panel C-I: IP growth, log(GPR), VIX + lags, mẫu 1985-2019.

    ``aligned=False`` giữ đúng timing của paper để replication. ``aligned=True``
    dịch GPR tháng M sang bucket M+1, dùng làm diagnostic riêng cho pipeline của
    dự án; không thay thế kết quả replication headline.
    """
    from pandas_datareader import data as pdr

    gpr = load_gpr_monthly()["GPR"].loc[SAMPLE[0]:SAMPLE[1]]
    if aligned:
        gpr = align_monthly_gpr_to_information_time(gpr)
...
def build_report(irf: pd.DataFrame, ev: dict, meta: dict) -> list[str]:
    ...
    parts = [
        ...
        f"- **Kết luận: {'replication headline ĐÚNG; panel M+1 phải đọc qua E0 aligned diagnostic riêng' if ev['passed'] else 'pipeline NGHI CÓ BUG — mọi kết luận G2a/lưới vô nghĩa cho tới khi sửa'}.**\n",
    ...
    parts += [
        ...
        ("Pipeline tái lập được C-I → loader + LP/HAC theo timing của paper hoạt động. "
         "Kết quả này KHÔNG đi qua alignment M+1 của lưới; chạy "
         "`scripts/run_e0_alignment_diagnostic.py` trước khi gỡ blocker timing của "
         "SCA-01." if ev["passed"] else
