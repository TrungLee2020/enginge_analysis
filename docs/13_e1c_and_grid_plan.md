...
E0 PASS xác nhận pipeline đúng (β=−0.37, p=0.008 tại h=2 tháng, IP giảm) → mọi kết quả null từ nay là **phát hiện thật**, không phải bug. Đây là điều kiện `docs/12` §9 yêu cầu trước khi tin bất kỳ null nào. Đường găng code đã thông.
...
|---|---|---|
| `LEVEL` | biến đổi đơn điệu | cao **theo định nghĩa** |
| `JUMP` = max(0, z − q95) | dương khi z vượt q95 | dương **theo định nghĩa** (97.5 > 95) |
| `LEVEL+JUMP` | chứa cả hai | cao **theo định nghĩa** |
| `INNOVATION` = LEVEL − AR(5) | **cố ý không** đơn điệu | không có lý do gì để cao |

Ranking `LEVEL+JUMP > JUMP > INNOVATION` là hệ quả của định nghĩa sự kiện. Report E1b gọi đây là "thiên vị nhẹ cho thước đo có thành phần level" — **nó không nhẹ, nó là toàn bộ kết quả.**
...
- `docs/reports/E1c_shock_detection_<data_version>.md`
...
`SCA-01.blockers` hiện chỉ liệt kê `holdout_decision_g0`. Thực tế còn ba việc nữa.
...
| Việc | Ghi chú |
|---|---|
| `build_monthly_panel()` | **Vẫn chưa tồn tại.** D2 cố ý chỉ build track daily. Đây là đầu vào bắt buộc của nửa lưới |
...
- [ ] `build_monthly_panel()` — song song, chặn nửa lưới
