...
| Chiều | Mức | Cơ sở |
|---|---|---|
| **SHOCK** | `INNOVATION` · `JUMP` · `LEVEL+JUMP` · `LEVEL` | E1/E1b: AR(5) có thể nén cú sốc. `LEVEL+JUMP` cho sốc dai dẳng. `LEVEL` thuần **thêm sau E0 PASS 2026-07-19** — spec headline C-I 2022 và là spec DUY NHẤT đã cho kết quả có ý nghĩa trong pipeline này (β=−0.37, p=0.008); thêm TRƯỚC khi lưới chạy (docs/13 §4.2) |
...
**Tổng: 4 × 2 × 6 × 2 × 2 × 2 = 384 spec** (SHOCK 4 mức sau khi thêm LEVEL, docs/13 §4.2).

Loại các tổ hợp không hợp lệ/trùng lặp trước khi chạy:
- `JUMP_THRESH` chỉ áp dụng khi shock có thành phần JUMP (`JUMP`, `LEVEL+JUMP`). Với `SHOCK ∈ {INNOVATION, LEVEL}` → giữ 1 mức JUMP_THRESH (không nhân đôi)
