...
Chạy lại đúng specification headline của Caldara–Iacoviello (tần suất, mẫu, biến, phương pháp như paper), rồi chạy thêm bản timing-aligned dùng GPR tháng M ở bucket M+1.

- Headline tái lập được → loader + LP/HAC theo timing paper hoạt động.
- `run_e0_alignment_diagnostic.py` PASS tại cặp neo raw h=2 → aligned h=1 → đường timing M+1 đủ điều kiện đi tiếp.
- Không tái lập được → **có bug**; mọi kết luận G2a vô nghĩa.

Hai phép thử cùng tạo thành gate cấp hệ thống. Headline PASS một mình không chứng nhận panel aligned của lưới.
